...
export async function ensureInfra(): Promise<void> {
  ...
  console.info("Starting Shannon infrastructure...");
  execFileSync("docker", ["compose", "-f", composeFile, "up", "-d"], {
    stdio: "inherit",
  });

  console.info("Waiting for Temporal to be ready...");
  for (let i = 0; i < 30; i++) {
    if (isTemporalReady()) {
...
export function buildImage(noCache: boolean): void {
  console.info(`Building ${DEV_IMAGE}...`);
  const args = ["build"];
  if (noCache) args.push("--no-cache");
  args.push("-t", DEV_IMAGE, ".");
  execFileSync("docker", args, { stdio: "inherit" });
  console.info(`Build complete: ${DEV_IMAGE}`);
}
...
export function ensureImage(version: string): void {
  ...
  if (getMode() === "local") {
    console.info("Shannon image not found, building...");
    buildImage(false);
  } else {
...
export function spawnWorker(opts: WorkerOptions): ChildProcess {
  ...
  if (os.platform() === "linux" && process.getuid && process.getgid) {
    args.push(
      "-e",
      `SHANNON_HOST_UID=${process.getuid()}`,
      "-e",
      `SHANNON_HOST_GID=${process.getgid()}`,
    );
  }
...
export function stopWorkers(): void {
  ...
  console.info("Stopping running scans...");
